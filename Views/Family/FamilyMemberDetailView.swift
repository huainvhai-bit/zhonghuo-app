...
struct FamilyMemberDetailView: View {
    ...
    private func acceptInvite() {
        Task {
            do {
                ...
                let result = try await DataManager.shared.acceptFamilyInvite(relationId: member.relationId)
                print("📡 接受邀请响应：\(result)")
                ...
                if success {
                    ...
                    let message = result["message"] as? String ?? "接受失败"
    ...
    private func rejectInvite() {
        Task {
            do {
                // ✅ 修复：使用 DataManager 统一函数
                let result = try await DataManager.shared.rejectFamilyInvite(relationId: member.relationId)
                print("📡 拒绝邀请响应：\(result)")
                ...
                if success {
                    ...
                    let message = result["message"] as? String ?? "拒绝失败"
    ...
    private func removeFamily() {
        Task {
            do {
                // ✅ 修复：使用 DataManager 统一函数
                let result = try await DataManager.shared.removeFamily(relationId: member.relationId)
                print("📡 移除家人响应：\(result)")
                ...
                if success {
                    ...
                    let message = result["message"] as? String ?? "移除失败"
