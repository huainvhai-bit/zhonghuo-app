...
/// Token 自动刷新服务
/// 职责：Token 过期时自动调用 refreshToken API 获取新 Token
class TokenManager {
    ...
    func isTokenExpiring() -> Bool {
        guard let expirationTime = KeychainManager.shared.getTokenExpiration() else {
            return false
        }
        return Date().timeIntervalSince1970 >= expirationTime.timeIntervalSince1970 - 300
    }
...
func refreshTokenAPI(_ refreshToken: String) async throws -> TokenRefreshResult {
    ...
    let baseURL = UserDefaults.standard.string(forKey: "serverURL") ?? "api.zhonghuo.app"
