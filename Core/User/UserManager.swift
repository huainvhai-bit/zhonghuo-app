...
//
//  用户管理 - 注册、登录、添加关系、定位
//  技术文档：📖 安伴助手 技术开发文档.md - 第 3 章 前端技术栈
//
...
class UserManager: NSObject, ObservableObject {
    ...
    func uploadLocation() {
        ...
        guard locationAuthStatus == .authorizedAlways || locationAuthStatus == .authorizedWhenInUse else {
            ...
            print("💡 请在 设置 → 隐私 → 定位服务 中允许安伴助手 使用定位")
            return
        }
    ...
    static func sendGraphQLQueryWithToken(query: String, variables: [String: Any]) async throws -> [String: Any] {
        let baseURL = NetworkUtils.normalizeBaseURL(UserDefaults.standard.string(forKey: "lastUsedBaseURL") ?? "zhonghuo.zhonghuo.xyz")
