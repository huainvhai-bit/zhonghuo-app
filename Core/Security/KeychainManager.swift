...
class KeychainManager {
    ...
    /// - Returns: 是否成功
    @discardableResult
    func saveToken(_ token: String, expiration: Int = 7200) -> Bool { // 默认 2 小时过期
        // 先删除旧的（如果有）
        deleteToken()
        ...
        if status == errSecSuccess {
            ...
            // 保存过期时间到 UserDefaults
            let expirationTime = Date().timeIntervalSince1970 + Double(expiration)
            UserDefaults.standard.set(expirationTime, forKey: tokenExpirationKey)
    ...
    /// 读取 Token 过期时间
    /// - Returns: 过期时间（Date），nil 表示未设置
    func getTokenExpiration() -> Date? {
        guard let timestamp = UserDefaults.standard.object(forKey: tokenExpirationKey) as? Double else {
            return nil
        }
    ...
    func saveRefreshToken(_ token: String) {
        saveItem(key: refreshTokenKey, value: token)
    }
    ...
    func clearToken() {
        ...
        print("✅ Keychain: 所有 Token 和用户信息已清空")
    }
}
