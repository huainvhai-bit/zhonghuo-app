...
struct HomeStatusView: View {
    ...
    var checkInStatus: (isSafe: Bool, hoursRemaining: Double) {
        dataManager.getCheckInStatus()
    }
    ...
    private var checkInCard: some View {
        VStack(spacing: 12) {
            ...
            Text(formatCountdown(checkInStatus.hoursRemaining))
            ...
            .disabled(checkInStatus.isSafe)
            .opacity(checkInStatus.isSafe ? 0.5 : 1)
        }
        .padding(20)
        .background(
            LinearGradient(
    ...
    private var quickActionsGrid: some View {
        LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible()), GridItem(.flexible()), GridItem(.flexible())], spacing: 12) {
            QuickActionItem(icon: "🎙️", label: "录制语音", color: Color(hex: "AF52DE"))
            QuickActionItem(icon: "📋", label: "身后事务", color: Color(hex: "34C759"))
            QuickActionItem(icon: "👥", label: "见证人", color: Color(hex: "FF9500"))
            QuickActionItem(icon: "💰", label: "资产", color: Color(hex: "007AFF"))
        }
        .padding(16)
        .background(Color.white)
        .cornerRadius(16)
